...
class PekerjaController extends Controller
{
    ...
    public function pekerja(Request $request)
    {
        ...
        $city = $request->input('city'); // Category filter
        ...
        if ($city) {
            $cityParts = explode(' ', $city); // Split by space
            $filteredCity = $cityParts[1] ?? $city; // Take the first part
        
            $pekerjaQuery->whereHas('user', function ($query) use ($filteredCity) {
                $query->where('regency_city', 'like', "%$filteredCity%");
            });
        }
        ...
        return view('landingpage.pekerja', compact('kategori', 'pekerja', 'pekerjaBaru', 'totalPekerjaCount', 'cities'));
    }
